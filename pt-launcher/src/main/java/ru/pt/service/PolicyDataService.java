...
public class PolicyDataService {
    ...
    /**
     * Create a new policy with both data and index
     * Extracts index fields from policy data using JSON paths
     */
    @Transactional
    public PolicyData createPolicy(Account account, String policy) {
        try {
            ObjectMapper objectMapper = new ObjectMapper();
            JsonNode policyData;
            try {
                policyData = objectMapper.readTree(policy);
            } catch (Exception e) {
                throw new BadRequestException("Invalid JSON for policyData", e);
            }
            // Get next policy ID
            Long policyId = policyDataRepository.getNextPolicyId();
            // Create and save PolicyData
            PolicyData data = new PolicyData();
            data.setId(policyId);
            data.setPolicy(policyData);
            policyDataRepository.save(data);
            
            // Create and save PolicyIndex
            PolicyIndex index = new PolicyIndex();
            index.setId(policyId);
            
            //objectMapper.configure(DeserializationFeature.FAIL_ON_UNKNOWN_PROPERTIES, false);
            //index = objectMapper.readValue(policy, PolicyIndex.class);


            // Extract fields from JSON using JsonPath
            DocumentContext ctx = JsonPath.parse(policy);
            
            try { index.setDraftId(ctx.read("$.draftId", String.class)); } catch (Exception e) {}
            try { index.setPolicyNr(ctx.read("$.policyNumber", String.class)); } catch (Exception e) {}
            try { index.setVersionNo(ctx.read("$.versionNo", Integer.class)); } catch (Exception e) {}
            //try { index.setTopVersion(ctx.read("$.topVersion", Boolean.class)); } catch (Exception e) { index.setTopVersion(false); }
            try { index.setProductCode(ctx.read("$.product.code", String.class)); } catch (Exception e) {}
            try { 
            ...
            try { index.setIssueTimezone(ctx.read("$.issueTimezone", String.class)); } catch (Exception e) {}
            try { 
            ...
            index.setUserAccountId( account.getId());

            //try { index.setPartnerAccountId(ctx.read("$.partnerAccountId", Long.class)); } catch (Exception e) {}
            try { index.setVersionStatus(ctx.read("$.versionStatus", String.class)); } catch (Exception e) { index.setVersionStatus("DRAFT"); }
            ...
            throw new BadRequestException("Error creating policy: " + e.getMessage());
        }
    }
    ...
    public PolicyData updatePolicy(Long policyId, JsonNode policyData) {
        try {
            ...
            try { index.setPolicyNr(ctx.read("$.policyNumber", String.class)); } catch (Exception e) {}
            try { index.setVersionNo(ctx.read("$.versionNo", Integer.class)); } catch (Exception e) {}
            try { index.setTopVersion(ctx.read("$.topVersion", Boolean.class)); } catch (Exception e) {}
            try { index.setProductCode(ctx.read("$.product.code", String.class)); } catch (Exception e) {}
            try { 
                String issueDateStr = ctx.read("$.issueDate", String.class);
                if (issueDateStr != null) {
            ...
            try { index.setUserAccountId(ctx.read("$.userAccountId", Long.class)); } catch (Exception e) {}
            //try { index.setPartnerAccountId(ctx.read("$.partnerAccountId", Long.class)); } catch (Exception e) {}
            try { index.setVersionStatus(ctx.read("$.versionStatus", String.class)); } catch (Exception e) {}
